...
printk("In slob_page_alloc\n");
	...
	if(best_fit != NULL){
		...
		best_fit_aligned = (slob_t *)ALIGN((unsigned long)best_fit, align); //!!
		best_fit_delta = aligned - best_fit; 		//!!
		...
		if (best_fit_avail == units) { /* exact fit? unlink. */
			if (prev_best_fit)
				set_slob(prev_best_fit, slob_units(prev), next_best_fit);
			else
